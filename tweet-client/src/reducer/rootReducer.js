...
const rootReducer = (state =defalut_state,action)=>{
    switch(action.type){
        ...
        case 'auto': return{...state,userId:action.payload.users.userId,profileImageUrl:action.payload.users.profileImageUrl,userName:action.payload.users.username,logout:true}
